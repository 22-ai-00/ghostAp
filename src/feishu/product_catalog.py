...
PUBLIC_ACTIONS: tuple[ProductAction, ...] = (
    ...
    _action("/ttadk", "进入 TTADK 多工具编程模式", aliases=("/enter_ttadk",), enters_programming_mode=True, programming_mode_id="ttadk", protects_from_auto_activation=True),
    _action("/tui2acp", "进入 TUI2ACP 桥接模式", aliases=("/enter_tui2acp",), enters_programming_mode=True, programming_mode_id="tui2acp", protects_from_auto_activation=True),
    _action("/acp", "打开 ACP 工具选择入口", enters_programming_mode=True, programming_mode_id="ttadk", protects_from_auto_activation=True),
    _action("/model", "查看或切换当前模型", "/model [list|模型名]"),
    _action("/exit", "退出当前编程模式", aliases=("/quit", "/end_coco", "/exit_coco", "/end_claude", "/exit_claude", "/end_aiden", "/exit_aiden", "/end_codex", "/exit_codex", "/end_gemini", "/exit_gemini", "/end_traex", "/exit_traex", "/end_ttadk", "/exit_ttadk", "/end_tui2acp", "/exit_tui2acp")),
    ...
    _action("/ttadk_info", "查看 TTADK 工具与模型信息"),
    _action("/tui2acp_info", "查看 TUI2ACP 会话信息"),
    _action("/ttadk_refresh", "刷新 TTADK 模型列表"),
    ...
    _action("/spec_pause", "暂停 Spec Engine 任务", protects_from_auto_activation=True),
    _action("/spec_resume", "继续 Spec Engine 任务", protects_from_auto_activation=True),
    ...
    _action("/wf_save", "保存 Workflow 模板", "/wf_save <名称>", aliases=("/workflow_save",)),
