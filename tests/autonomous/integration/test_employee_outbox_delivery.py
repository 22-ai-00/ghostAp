...
def test_targeted_task_routing_failure_is_terminal_and_idempotent_across_replay(
    tmp_path,
) -> None:
    service, writer, anchor = _runtime(tmp_path)
    lifecycle = EmployeeOutboxLifecycle(service)
    try:
        first = lifecycle.task_failure_response(
            tenant_key="tenant-a",
            agent_id="agt_alpha",
            chat_id="oc_team",
            thread_root_message_id="om_task",
            command_acceptance_id="acc_task_routing_failure",
        )
        repeated = lifecycle.task_failure_response(
            tenant_key="tenant-a",
            agent_id="agt_alpha",
            chat_id="oc_team",
            thread_root_message_id="om_task",
        ...
        assert first.state is EmployeeCardState.ACTION_REQUIRED
        assert first.terminal_version == 2
        assert "本次未执行" in first.card_json["body"]["elements"][0]["content"]
        assert len(service.get_record(first.outbox_id).snapshots) == 2
    finally:
        service.close()
        writer.close()
