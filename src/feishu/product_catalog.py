...
PUBLIC_ACTIONS: tuple[ProductAction, ...] = (
    ...
    _action("/acp", "打开 ACP 工具选择入口", enters_programming_mode=True, protects_from_auto_activation=True),
    _action("/model", "查看或切换当前模型", "/model [list|模型名]"),
    _action("/exit", "退出当前编程模式", aliases=("/quit", "/end_coco", "/exit_coco", "/end_claude", "/exit_claude", "/end_aiden", "/exit_aiden", "/end_codex", "/exit_codex", "/end_gemini", "/exit_gemini", "/end_traex", "/exit_traex", "/end_tui2acp", "/exit_tui2acp")),
...
_EXECUTION_LANE_DESCRIPTORS: tuple[ExecutionLaneDescriptor, ...] = (
    ExecutionLaneDescriptor(
        ...
        ("coco", "claude", "aiden", "codex", "gemini", "traex", "tui2acp"),
    ),
